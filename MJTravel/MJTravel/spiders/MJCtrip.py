...
class MjctripSpider(scrapy.Spider):
    ...
    def parse(self, response):
       travel_sel = Selector(response)
#       link_browse = travel_sel.select("//a[@class='link_browse']/span/text()").extract()
       link = response.url
...
#       b_count = response.xpath("//a[@class='link_browse']/span/text()").extract()
       b_count= travel_sel.xpath("//a[@class='link_browse']/span/text()").extract()
       print "++++++++++++", b_count
       if len(b_count) >= 1: 
